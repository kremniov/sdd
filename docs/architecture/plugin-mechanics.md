...
lands in the project. A scaffold file lands in the project once and is never
read again.
...
locations from `.sdd.yml`. The illustrative config block in `adopting-sdd` is
not an instruction and is the one exemption.

## The order that matters

1. `adopting-sdd` writes `.sdd.yml` **first** — every later skill reads it, and
...
4. `deriving-canon` runs after adoption, never before: it fills the body of an
